...
const DEFAULT_FROM_EMAIL =
  process.env.DEFAULT_FROM_EMAIL || "onboarding@resend.dev"; // Fallback, replace with your verified Resend domain email
...
export async function POST(
  ...
  try {
    ...
    const { senderName, senderEmail, message }: ContactFormValues =
    ...
    if (!link.deliveryOptions || link.deliveryOptions.length === 0) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: "No delivery options configured for this link.",
        },
    ...
    for (const option of link.deliveryOptions) {
      if (option.type === "EMAIL") {
        console.log(
          `Attempting to send email to: ${option.destination} for link: ${slug}`
        );
        deliveryPromises.push(
          resend.emails
            ...
            .then((response) => {
              if (response.error) {
                console.error(
                  `Resend error for ${option.destination}:`,
                  response.error
                );
                return {
                  success: false,
            ...
            .catch((error) => {
              console.error(
                `Failed to send email to ${option.destination}:`,
                error
              );
              return {
                success: false,
        ...
        console.log(
          `Attempting to send webhook to: ${option.destination} for link: ${slug}`
        );
          ...
          fetch(option.destination, {
            ...
            .then(async (response) => {
              if (!response.ok) {
                ...
                console.error(
                  `Webhook error for ${option.destination}: ${errorBody}`
                );
                return {
                  success: false,
                  type: "WEBHOOK",
              ...
              console.log(
                `Webhook sent successfully to: ${option.destination}`
              );
              return {
                success: true,
                type: "WEBHOOK",
            ...
            .catch((error) => {
              console.error(
                `Failed to send webhook to ${option.destination}:`,
                error
              );
              return {
                success: false,
    ...
    if (deliveryPromises.length === 0) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: "No email delivery options were processed for this link.",
        },
        { status: 400 }
      );
    }
    ...
    const successfullySent = results.some(
    ...
    return NextResponse.json(
      {
        success: false,
        error: "Failed to deliver message to configured email destinations.",
      },
      { status: 500 }
    );
  } catch (error) {
