...
export default function ContactForm({ linkId, linkSlug }: ContactFormProps) {
  ...
  const onSubmit = async (values: ContactFormValues) => {
    ...
    try {
      ...
      const response = await fetch(`${linkSlug}`, {
        method: "POST",
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <FormField
          ...
          name="senderName"
          render={({ field }: { field: ControllerRenderProps<ContactFormValues, "senderName"> }) => (
            <FormItem>
        ...
        <FormField
          control={form.control}
          name="senderEmail"
          render={({ field }: { field: ControllerRenderProps<ContactFormValues, "senderEmail"> }) => (
            <FormItem>
        ...
        <FormField
          ...
          render={({ field }: { field: ControllerRenderProps<ContactFormValues, "message"> }) => (
            <FormItem>
              ...
              <FormControl>
                <Textarea placeholder="Your message..." rows={5} {...field} disabled={isLoading} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        {serverMessage && (
          <p className="text-sm font-medium text-emerald-600 text-center">{serverMessage}</p>
        )}
